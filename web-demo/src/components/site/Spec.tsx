    ...
    <section id="how" className="mx-auto max-w-5xl px-6 py-20">
      <Reveal>
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-white/10 bg-black/60 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
          <div className="font-mono text-[11px] leading-6 text-slate-500">// newline-delimited frames</div>
...
export function HardwareSpec() {
  return (
    <section id="hardware" className="mx-auto max-w-5xl px-6 py-20">
      <Reveal>
        <h2 className="max-w-xl text-2xl font-bold tracking-tight text-slate-50 sm:text-3xl">
          Plug into the suit.
        </h2>
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-3">
          {FEATURED.map((f) => (
            ...
            >
              ...
              <div className="mt-1 text-[11px] font-semibold tracking-[0.18em] text-slate-500">
                {f.label.toUpperCase()}
              </div>
            </div>
          ))}
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {CLUSTERS.map((c) => (
            <div key={c.title} className="rounded-2xl border border-slate-800 bg-bio-panel/60 p-5">
              <div className="text-[11px] font-bold tracking-[0.22em] text-slate-400">
                {c.title.toUpperCase()}
              </div>
              <dl className="mt-3 space-y-2.5">
                {c.rows.map(([k, v]) => (
                  <div key={k}>
                    <dt className="font-mono text-[10px] tracking-wider text-slate-600">{k}</dt>
                    <dd className="break-all font-mono text-xs text-slate-300">{v}</dd>
                  </div>
                ))}
              </dl>
            </div>
        ...
        <p className="mt-5 max-w-[65ch] text-[12px] leading-relaxed text-slate-600">
